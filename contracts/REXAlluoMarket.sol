...
contract REXAlluoMarket is Ownable, SuperAppBase, Initializable {
    ...
    ISuperToken inputToken;  // e.g. stibAlluoUSD
    ISuperToken outputToken; // e.g. stibAlluoETH
    ISuperToken subsidyToken; // e.g. RICx
    address inputTokenERC20; // e.g. USDC
    address outputTokenERC20; // e.g. WETH
        ...
        returns (bytes memory newCtx)
    {
        ...
        IbAlluo ibInputToken = IbAlluo(_getUnderlyingToken(inputToken));
        IbAlluo ibOutputToken = IbAlluo(_getUnderlyingToken(outputToken));

        // Downgrade stIbAlluoX -> ibAlluoX
        inputToken.downgrade(inputToken.balanceOf(address(this)));
        ibInputToken.updateRatio();
        uint256 inputTokenERC20Amount = ibInputToken.balanceOf(address(this)) * ibInputToken.growingRatio() / 1e18;
        
        // Withdraw ibAlluoX -> inputTokenERC20
        ibInputToken.withdraw(
            inputTokenERC20,
            inputTokenERC20Amount
        );

        // Swap inputTokenERC20 -> outputTokenERC20
        uint256 outputTokenERC20Amount = _swap(inputTokenERC20Amount);

        // Record the exchange rate
        _recordExchangeRate(inputTokenERC20Amount * 1e18 / outputTokenERC20Amount, block.timestamp);

        // Deposit outputTokenERC20 -> ibAlluoY
        ibOutputToken.deposit(
            outputTokenERC20,
            ERC20(outputTokenERC20).balanceOf(address(this))
        );

        // Upgrade ibAlluoY -> stIbAlluoY
        outputToken.upgrade(ibOutputToken.balanceOf(address(this)));

        // All inputTokens have been swapped to outputToken

        // Calculate the amount of outputToken to distribute
