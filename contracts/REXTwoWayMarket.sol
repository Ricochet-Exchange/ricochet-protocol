...
contract REXTwoWayMarket is REXMarket {
    ...
    ISuperToken public inputTokenA;
    ISuperToken public inputTokenB;
    IUniswapV2Router02 public router;
    uint32 public constant OUTPUTA_INDEX = 0;
    uint32 public constant OUTPUTB_INDEX = 1;
    mapping(ISuperToken => uint32) public outputPoolIndices;
    ...
    ) REXMarket(_owner, _host, _cfa, _ida, _registrationKey, _rexReferral) {} // solhint-disable-line no-empty-blocks
        ...
        returns (bytes memory newCtx)
    {
        ...
        require(
            market
                ...
                .lastUpdatedAt >= block.timestamp - 1 hours,
            "!currentValueA"
        );
        require(
            market
        ...
        console.log(
            "inputTokenA Balance: ",
            inputTokenA.balanceOf(address(this))
        );
        console.log(
            "inputTokenB Balance: ",
            inputTokenB.balanceOf(address(this))
        );

        // Check how much inputTokenA we have already from tokenB
        uint256 tokenHave = (inputTokenB.balanceOf(address(this)) *
        ...
        if (tokenAAmount > 0 && _totalUnitsApproved + _totalUnitsPending > 0) {
            ...
            // Distribute TokenA
            (feeCollected, distAmount) = _getFeeAndDist(
                tokenAAmount,
                market.outputPools[OUTPUTA_INDEX].feeRate
            );
        ...
        if (tokenBAmount > 0 && _totalUnitsApproved + _totalUnitsPending > 0) {
            ...
            (feeCollected, distAmount) = _getFeeAndDist(
                tokenBAmount,
                market.outputPools[OUTPUTB_INDEX].feeRate
            );
    ...
    function _getFeeAndDist(uint256 tokenAmount, uint256 feeRate)
        internal
        pure
        returns (uint256 feeCollected, uint256 distAmount)
    {
        ...
        if (_totalUnitsApproved + _totalUnitsPending > 0) {
            ...
            if (_totalUnitsApproved + _totalUnitsPending > 0) {
                ...
                uint256 _balance = ISuperToken(inputTokenA).balanceOf(
                    ...
                    (10 **
                        (18 -
                            ERC20(inputTokenA.getUnderlyingToken())
                                .decimals()));
