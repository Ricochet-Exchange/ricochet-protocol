  ...
  OpsTaskCreator
{
  ...
  event RexSwap(uint256 inputAmount, uint256 outputAmount, uint256 oraclePrice);
  ...
  function distribute(
    ...
    if (distAmount > 0 && distAmount < subsidyToken.balanceOf(address(this))) {
      newCtx = _idaDistribute(
        SUBSIDY_INDEX,
        uint128(distAmount),
        subsidyToken,
  ...
  function _shouldDistribute() internal view returns (bool) {
    ...
    (, , uint128 _totalUnitsApproved, uint128 _totalUnitsPending) = ida
      .getIndex(outputPools[OUTPUT_INDEX].token, address(this), OUTPUT_INDEX);
    ...
    returns (uint128 userShares, uint128 daoShares, uint128 affiliateShares)
  {
    ...
    if (changeInFlowRate > 0) {
      ...
      feeShares = uint128((uint256(int256(changeInFlowRate)) * feeRate) / 1e6);
      if (address(0) != _shareholderUpdate.affiliate) {
        affiliateShares -= ((feeShares * affiliateFee) / 1e6 > affiliateShares)
          ? affiliateShares
