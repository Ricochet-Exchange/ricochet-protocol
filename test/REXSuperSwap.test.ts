...
describe('REXSuperSwap', () => {
  ...
  async function takeMeasurements(balances: SuperTokensBalances, signer: SignerWithAddress): Promise<void> {
    appBalances.ethx.push(
      (await superT.ethx.balanceOf({ account: superSwap.address, providerOrSigner: provider })).toString()
    )
    ...
    appBalances.usdcx.push(
      (await superT.usdcx.balanceOf({ account: superSwap.address, providerOrSigner: provider })).toString()
    )
    ...
    appBalances.ric.push(
      (await superT.ric.balanceOf({ account: superSwap.address, providerOrSigner: provider })).toString()
    )
    ...
    appBalances.maticx.push(
      (await superT.maticx.balanceOf({ account: superSwap.address, providerOrSigner: provider })).toString()
    )
    ...
    aliceBalances.daix.push(
      (await superT.daix.balanceOf({ account: u.alice.address, providerOrSigner: provider })).toString()
    )
    aliceBalances.wbtcx.push(
      (await superT.wbtcx.balanceOf({ account: u.alice.address, providerOrSigner: provider })).toString()
    )
  }
  ...
  context('#1 Test swap functionality', async () => {
    it('#1.1 User can swap token maticx -> usdcx', async () => {
      ...
      // call swap function
      const swapTx = await superSwap
        .connect(aliceSigner)
        .swap(from, to, amountIn, amountOutMin, path, poolFees, true, true)
    ...
    it('#1.3 User can swap native token RIC -> daix', async () => {
      ...
      // call swap function
      const swapTx = await superSwap.connect(aliceSigner).swap(from, to, amountIn, 0, path, poolFees, false, true)
    ...
    it('#1.6 User can swap large amount of token USDCX -> WBTCX', async () => {
      ...
      // hardcoding for now
      const amountToSwap = 200 * 0.00005
      const percentage = (amountToSwap / 100) * 3
      const amount = amountToSwap - percentage
      const amountOutMin = Math.round(amount)
