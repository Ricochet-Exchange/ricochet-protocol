...
describe('REXTwoWayAlluoUsdcxMarket', () => {
  ...
  async function takeMeasurements(balances: SuperTokensBalances, signer: SignerWithAddress): Promise<void> {
    ...
    appBalances.ricochetUSDCx.push(
      (await ricochetUSDCx.balanceOf({ account: twoWayMarket.address, providerOrSigner: provider })).toString()
    )
    ownerBalances.ricochetUSDCx.push(
      (await ricochetUSDCx.balanceOf({ account: u.admin.address, providerOrSigner: provider })).toString()
    )
    aliceBalances.ricochetUSDCx.push(
      (await ricochetUSDCx.balanceOf({ account: u.alice.address, providerOrSigner: provider })).toString()
    )
    carlBalances.ricochetUSDCx.push(
      (await ricochetUSDCx.balanceOf({ account: u.carl.address, providerOrSigner: provider })).toString()
    )
    karenBalances.ricochetUSDCx.push(
      (await ricochetUSDCx.balanceOf({ account: u.karen.address, providerOrSigner: provider })).toString()
    )
    bobBalances.ricochetUSDCx.push(
      (await ricochetUSDCx.balanceOf({ account: u.bob.address, providerOrSigner: provider })).toString()
    )
  ...
  before(async () => {
    ...
    REXMarketFactory = await ethers.getContractFactory('REXTwoWayAlluoUsdcxMarket', adminSigner)
    ...
    await twoWayMarket.initializeTwoWayMarket(ricochetUSDCx.address, 1, stIbAlluoUSD.address, 1, 0, 20000)
    ...
    await approveSubscriptions(
      [stIbAlluoUSDAndItsIDAIndex, ricAndItsIDAIndex],
  ...
  context.only('#1 - new rexmarket with no streamers', async () => {
    ...
    it('#1.2 before/afterAgreementCreated callbacks', async () => {
      ...
      expect((await twoWayMarket.getIDAShares(IBALLUOUSD_SUBSCRIPTION_INDEX, aliceSigner.address)).toString()).to.equal(
        `true,true,1000000000000000,0`
      )
      // No fees
      ...
      await sf.cfaV1
        ...
        .exec(bobSigner)
      // Expect share allocations were done correctly
      expect(await twoWayMarket.getStreamRate(bobSigner.address, ricochetUSDCx.address)).to.equal(inflowRateUsdc)
      expect((await twoWayMarket.getIDAShares(IBALLUOUSD_SUBSCRIPTION_INDEX, bobSigner.address)).toString()).to.equal(
        `true,true,1000000000000000,0`
      )
    })
    ...
    it('#1.3 before/afterAgreementTerminated callbacks', async () => {
      ...
      expect(await twoWayMarket.getStreamRate(bobSigner.address, ricochetUSDCx.address)).to.equal('0')
      expect((await twoWayMarket.getIDAShares(IBALLUOUSD_SUBSCRIPTION_INDEX, aliceSigner.address)).toString()).to.equal(
        `true,true,0,0`
      )
      expect((await twoWayMarket.getIDAShares(IBALLUOUSD_SUBSCRIPTION_INDEX, bobSigner.address)).toString()).to.equal(
    ...
    it('#1.4 one-sided distribution USDCx > stIbAlluoUSD', async () => {
      ...
      expect(
        ((-1 * deltaAlice.ricochetUSDCx) / deltaAlice.stIbAlluoUSD - realGrowingRatio) / realGrowingRatio
      ).to.be.below(0.0003)
      expect(((-1 * deltaBob.ricochetUSDCx) / deltaBob.stIbAlluoUSD - realGrowingRatio) / realGrowingRatio).to.be.below(
        0.0003
      )
    })
  })
  ...
  xcontext('#3 - market is jailed', async () => {
    ...
    it('#3.1 emergencyCloseStream', async () => {
      ...
      expect(await twoWayMarket.getStreamRate(bobSigner.address, ibAlluoETH.address)).to.equal(0)
    })
