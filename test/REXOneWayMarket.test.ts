...
import { getSeconds, increaseTime, impersonateAccounts } from "./../misc/helpers";
import { Superfluid__factory } from "@superfluid-finance/sdk-core/dist/main/typechain";
...
//"0xB0aABBA4B2783A72C52956CDEF62d438ecA2d7a1",
    ...
    await tp.submitValue(parseInt(constant.TELLOR_ETH_REQUEST_ID), oraclePrice);
    await tp.submitValue(parseInt(constant.TELLOR_USDC_REQUEST_ID), 1000000);
  });
  ...
  it("should create a stream exchange with the correct parameters", async () => {
    ...
    console.log('usdcx: ', (await superT.usdcx.balanceOf({account: u.admin.address, providerOrSigner: accountss[0]})).toString());
    console.log('usdcx: ', (await superT.usdcx.balanceOf({account: u.app.address, providerOrSigner: accountss[0]})).toString());
    ...
    const txnResponse = await sf.cfaV1.createFlow({
  ...
  it.only("approval should be unlimited", async () => {
    const appSigner = await impersonateAccounts([u.app.address]);
    await sf.idaV1.approveSubscription({indexId: "0", superToken: superT.usdcx.address })
    const approveTxn = await superT.usdcx
      .approve({
        receiver: constant.SUSHISWAP_ROUTER_ADDRESS,
        amount: ethers.constants.MaxUint256.toString(),
      })
      .exec(accountss[0]);
    const approveTxnReceipt = await approveTxn.wait();
    ...
    expect(
      await superT.ethx.allowance({owner: u.admin.address, spender: constant.SUSHISWAP_ROUTER_ADDRESS, providerOrSigner: accountss[0]})
  ...
  //   const bobUsdcxBalance = new BN(await superT.usdcx.balanceOf({account: u.bob.address, providerOrSigner: accountss[3]}));
  ...
  //   await superT.usdcx.transfer(u.alice.address, toWad(400), { from: u.spender.address });
  //   console.log('Transfer bob');
  //   await superT.usdcx.transfer(u.bob.address, toWad(400), { from: u.spender.address });
  //   console.log('Done');
